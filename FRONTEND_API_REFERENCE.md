# Cardiotox TdP Screening API Reference

Welcome to the frontend API reference! This backend powers the Cardiotox TdP screening app, providing secure authentication, role-based access control, an admin panel, and an AI prediction proxy communicating with a Gradio model.

**Base URLs**:
- **Local Development**: `http://localhost:3000`
- **Production**: `https://cardiotox-backend.onrender.com`

---

> [!IMPORTANT] 
> **TL;DR / Golden Rules**
> - **No Tokens**: We use secure, HTTP-only cookies. You MUST send `credentials: 'include'` (fetch) or `withCredentials: true` (axios) with **every** request.
...
- There is no token to store in `localStorage`. The backend sets an `HttpOnly`, `Secure`, `SameSite=None` cookie upon successful login.
- **Cross-Origin Requests (CORS)**: Because the frontend and backend are on different domains in production, you must explicitly tell your HTTP client to send the cookie.
- **HTTPS Required**: Browsers will reject `SameSite=None` cookies if the connection is not secure (HTTPS). Localhost is usually exempted, but production requires it.
...
- Check `role === 'admin'` to show or hide the Admin Dashboard button in your navigation.
...
- **Errors**: `400 Bad Request` ("no password set for this account; you sign in with Google.")
...
#### POST `/api/predict`
Runs the Cardiotox classification model.
- **Access**: Protected
- **Body**:
  ```json
  {
...
- **Success (200 OK)**:
  ...
  ```
  > **Note**: Always read the classification tier from `data[0].label`. Ignore the raw string in `data[1]` as it is inconsistently formatted by Gradio.
...
| :--- | :--- | :--- |
| **400** `Bad Request` | Validation failed, missing fields, or invalid/expired 6-digit code. | Show the error text to the user. |
...
4. **Admin Dashboard (Protected)**:
   ...
   - Global Activity Feeds (auth events and prediction logs).
