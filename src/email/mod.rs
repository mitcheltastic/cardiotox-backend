...
impl Mailer {
    ...
    async fn send_resend(&self, to: &str, subject: &str, text: &str, html: &str) -> bool {
        let payload = json!({
            "from": self.from,
            "to": [to],
            "subject": subject,
    ...
    pub async fn send_verification(&self, to: &str, verify_link: &str) -> Result<()> {
        ...
        if self.send_resend(to, subject, &text, &html).await {
    ...
    pub async fn send_action_code(&self, to: &str, code: &str, action_label: &str) -> Result<()> {
        let subject = format!("Your code for {}", action_label);
        let text = format!("Your 6-digit code is: {}\n\nThis code expires in 10 minutes. If you didn't request this, ignore this email.", code);
        let html = action_code_html(action_label, code);

        if self.send_resend(to, &subject, &text, &html).await {
            info!("Action code email sent for {}", action_label);
        }
        Ok(())
    }
}
...
fn action_code_html(action_label: &str, code: &str) -> String {
    format!(
        r#"<!DOCTYPE html>
<html>
<head>
